...
app.use(async function(req, res, next) {
 ...
 if (subdomains.length > 1 || !req.server) return res.redirect("http://" + req.get('host').split(".").slice(subdomains.length).join("."))
...
app.listen(PORT, () => console.log(`Site online! (port ${PORT})`))
